                ...
                <Popover.Panel className="absolute inset-x-0 top-0 bg-white/95 py-3.5 shadow-sm [@supports(backdrop-filter:blur(0))]:bg-white/80 [@supports(backdrop-filter:blur(0))]:backdrop-blur">
                  ...
                  >
                    {(sectionIndex + 1).toString().padStart(2, '0')}
                        ...
                        <span className="ml-4 font-sans text-slate-900">
                    {section.title}
                  </span>
                      </Popover.Button>
                  ))}
            ...
            {sections.map((section, sectionIndex) => (
                <li key={section.id} className="flex [counter-increment:section]">
                  <Link
                      href={`#${section.id}`}
                      className={clsx(
                          'flex w-full font-sans flex-col items-center justify-center border-b-2 before:mb-2 before:font-sans before:text-sm before:content-[counter(section,decimal-leading-zero)]',
                          sectionIndex === activeIndex
                              ? 'border-blue-600 bg-blue-50 font-sans text-blue-600 before:text-blue-600'
                              : 'border-transparent before:text-slate-500 font-sans hover:bg-blue-50/40 hover:before:text-slate-900'
                      )}
                  >
                    {section.title}
                  </Link>
